...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        # print "ELIMINATION ORDER: " , eliminationOrder , "\n"
        for eliminationVariable in eliminationOrder:
            currentFactorsList = singleElimination(currentFactorsList, eliminationVariable, joinFactorsByVariable, eliminate)
        # joining over factors and normalizing makes it conditioned
        # P(A,+b) => normalize => P(A|+b)
        currentFactor = joinFactors(currentFactorsList)
        currentFactor = normalize(currentFactor)
        return currentFactor
        # DONE ELIMINATING
        
        
    def singleElimination(currentFactorsList, eliminationVariable, joinFactorsByVariablez, eliminatez):
        # print "-- START JOIN --"
        # print currentFactorsList
        # print "eliminationVariable: " , eliminationVariable
        currentFactorsList , joinedFactor = joinFactorsByVariablez(currentFactorsList,eliminationVariable)
        # print "-- AFTER JOIN --"
        # print "CURRENTFACTORSLIST: " , currentFactorsList , "\n"
        # print "joinedFactor: " , joinedFactor , "\n"

        if len(joinedFactor.unconditionedVariables()) > 1 and (eliminationVariable in joinedFactor.unconditionedVariables()):
            joinedFactor = eliminatez(joinedFactor, eliminationVariable)
            # print "AFTER ELIMINATE: " , joinedFactor
            currentFactorsList.append(joinedFactor)
