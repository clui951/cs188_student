...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        repeated_amount = int(self.numParticles/len(self.legalPositions))
        remainder = self.numParticles % len(self.legalPositions)

        # Empty self.particles
        self.particles = []

        # Generate particles
        for pos in self.legalPositions:
            for x in range(repeated_amount):
                self.particles.append(pos)

        # Generate more particles using remainder
        for x in range(remainder):
            self.particles.append(self.legalPositions[x])
    ...
    def observe(self, observation, gameState):
        ...
        # Ghost not captured yet, continue to search
        else:
            newParticles = util.Counter()
            # Weight all particles according to emissionModel
            for p in self.particles:
                dist = util.manhattanDistance(p, pacmanPosition)
                newParticles[p] = newParticles[p] + emissionModel[dist]

            #self.beliefs = newParticles
            self.particles = newParticles

            # Check for sum of all weights being equal to 0
            if self.particles.totalCount() == 0:
                self.initializeUniformly(gameState)
                self.particles = self.getBeliefDistribution() # Simply so normalizing step (not necessary in this case...) won't error out

            # Normalize values; from weighted probs to normalized
            self.particles.normalize()

            # Resample
            resampled = []
            for x in range(self.numParticles):
                #resampled.append(util.sample(self.beliefs))
                resampled.append(util.sample(self.particles))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # All possible permutations of legal ghost positions as a list of tuples
        permutations = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        repeated_amount = int(self.numParticles/len(permutations))
        remainder = self.numParticles % len(permutations)

        # Empty self.particles
        self.particles = []

        # Scramble permutations
        random.shuffle(permutations)

        # Add numParticles of the permutated particles to self.particles

        # Add clean number of particles
        for perm in permutations:
            for x in range(repeated_amount):
                self.particles.append(perm)

        # Generate more particules using remainder
        for x in range(remainder):
    ...
    def observeState(self, gameState):
        ...
        def putGhostsInJail(numGhosts, noisyDistances):
            for i in range(self.numGhosts):
                # Ghost captured
                if noisyDistances[i] is None:
                    self.particles = [self.getParticleWithGhostInJail(x, i) for x in self.particles]

        # Put ghosts in jail if they need to be
        putGhostsInJail(self.numGhosts, noisyDistances)

        newParticles = util.Counter()

        # Weight all particles according to emissionModels
        for p in self.particles:
            particle_weight = 1
            for i in range(self.numGhosts):
                ghostPos = p[i]
                dist = util.manhattanDistance(ghostPos, pacmanPosition)
                particle_weight = particle_weight*emissionModels[i][dist]
            newParticles[p] = newParticles[p] + particle_weight
        ...
        if self.particles.totalCount() == 0:
            self.initializeParticles()
            # Re-set particles to agree with any ghosts already in jail
            putGhostsInJail(self.numGhosts, noisyDistances)
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                # just pass in the entire particle
